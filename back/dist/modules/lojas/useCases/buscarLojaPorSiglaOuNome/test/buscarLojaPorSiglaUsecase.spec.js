...
var BuscarLojaPorSiglaOuNomeUsecase = class {
  ...
  async execute(sigla2) {
    const loja = await this.lojasRepository.buscarPorSiglaOuNome(sigla2);
    if (!loja) {
...
// src/modules/lojas/useCases/buscarLojaPorSiglaOuNome/test/buscarLojaPorSiglaUsecase.spec.ts
var lojaRepository;
var sut;
...
describe("Buscar Loja Por Sigla", () => {
  ...
  it("Deve encontrar uma loja pela Sigla da loja", async () => {
    ...
    globalExpect(loja?.sigla).toEqual("sigla-teste");
    globalExpect(loja?.pastaWEB).toEqual("uF-loja");
  });
  it("N\xE3o deve achar uma loja no sistema", async () => {
    globalExpect(async () => {
